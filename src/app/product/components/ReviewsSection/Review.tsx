...
const ReviewRepliesContainer: any = styled(Column)`
	width: 100%;
	max-height: ${({ reviewsVisibility }:  any) => reviewsVisibility ? '1000px' : 0};
...
const ToggleReviewsArrow:  any = styled.div`
	...
	transform: 	${({ reviewsVisibility }: any) => reviewsVisibility ? 'rotate(225deg)' : 'rotate(45deg)'};
	border-top: none;
	border-left: none;
	margin-left: 10px;
	margin-bottom: -5px;
					...
					<ReviewRepliesContainer reviewsVisibility={reviewsVisibility}>
						{
							props.replies && !!props.replies.length && (
								<ArrowContainer onClick={() => setReviewsVisibility(currentValue => !currentValue)}>
