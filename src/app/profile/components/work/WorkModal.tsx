...
export function WorkModal(props) {
  ...
  function onDelete() {
    deleteWorkExperience(editValues.id);
  }
    ...
    <Fragment>
      <Modal backdrop="static" show={isShow} onHide={onHide}>
        <Modal.Header closeButton>
          <Modal.Title>
            {editValues ? 'Edit experience' : 'Add experience'}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Formik
            initialValues={
              editValues
                ? {
                    ...editValues,
                    end_date: new Date(moment( `30/${editValues.end_date}`, 'DD/MM/YYYY', ).format()),
                    start_date: new Date(moment(`30/${editValues.end_date}`,'DD/MM/YYYY',).format()),
                  }
                : { ...initialValues }
            }
